...
business_form_router = APIRouter(prefix="/business", tags=["BusinessForm"])
...
async def get_business_form(
    ...
    return BusinessFormSchema.model_validate(business_form.scalars().first())
...
async def create_business_form(
    ...
    return BusinessFormSchema(profile_id=business_form.profile_id)
...
async def update_business_form(
    ...
    await manager.update_business_form(profile_id=profile.profile_id, update_data=update_data)
    return Response(status_code=200, content="Update profile successful")
